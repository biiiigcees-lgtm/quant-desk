...
export interface ReplayVerifyResult {
  ...
  readonly duplicates: number[]; // Duplicate sequence numbers
...
export class ReplayStorage {
   ...
   */
  async verify(): Promise<ReplayVerifyResult> {
   ...
   */
  async replay(
    handler: (record: ReplayRecord) => void | Promise<void>,
    ...
    const filtered = records
      .filter((r) => {
        if (options.fromSequence !== undefined && r.sequence < options.fromSequence) return false;
        if (options.toSequence !== undefined && r.sequence > options.toSequence) return false;
        return true;
      })
