...
export const executionIntelligenceAgent = {
    kind: 'execution-intelligence',
    preferredModels: AGENT_MODEL_POLICY['execution-intelligence'],
    debounceMs: 750,
    cacheTtlMs: 2500,
    buildSystemPrompt: () => makeAgentSystemPrompt('Execution Intelligence Agent (optimize how to execute, never whether to trade)', '{"orderStyle":"market|passive|sliced","slices":1-10,"timingMs":0-5000,"expectedSlippage":0-1,"fillProbability":0-1,"confidence":0-1,"missing_data":boolean}'),
    buildUserPrompt: (context) => `Propose execution tactics for ${context.contractId}. Trigger=${context.triggerEvent}. Input=${makeCompactContext(context.payload)}`,
    parseOutput: (raw) => {
        const parsed = parseAgentJson(raw);
        const orderStyle = parsed.orderStyle === 'passive' || parsed.orderStyle === 'sliced' ? parsed.orderStyle : 'market';
        return {
            orderStyle,
