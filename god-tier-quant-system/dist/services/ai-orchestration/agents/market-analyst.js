...
export const marketAnalystAgent = {
    kind: 'market-analyst',
    preferredModels: AGENT_MODEL_POLICY['market-analyst'],
    debounceMs: 1500,
    cacheTtlMs: 5000,
    buildSystemPrompt: () => makeAgentSystemPrompt('Market Analyst Agent (read-only market interpretation)', '{"narrative":"string","regimeClassification":"string","keyObservations":["string"],"confidence":0-1,"missing_data":boolean}'),
    buildUserPrompt: (context) => `Analyze market state for contract ${context.contractId}. Trigger=${context.triggerEvent}. Input=${makeCompactContext(context.payload)}`,
    parseOutput: (raw) => {
