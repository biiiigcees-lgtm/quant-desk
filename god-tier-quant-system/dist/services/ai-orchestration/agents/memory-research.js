...
export const memoryResearchAgent = {
    kind: 'memory-research',
    preferredModels: AGENT_MODEL_POLICY['memory-research'],
    debounceMs: 4000,
    cacheTtlMs: 12000,
    buildSystemPrompt: () => makeAgentSystemPrompt('Memory Research Agent (historical analog retrieval and comparison)', '{"similarHistoricalRegimes":[{"tag":"string","similarity":0-1,"outcome":"string"}],"structuralSimilarities":["string"],"confidence":0-1,"missing_data":boolean}'),
    buildUserPrompt: (context) => `Retrieve historical analogs for ${context.contractId}. Trigger=${context.triggerEvent}. Input=${makeCompactContext(context.payload)}`,
    parseOutput: (raw) => {
