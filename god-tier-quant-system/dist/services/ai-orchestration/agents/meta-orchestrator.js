...
export const metaOrchestratorAgent = {
    kind: 'meta-orchestrator',
    preferredModels: AGENT_MODEL_POLICY['meta-orchestrator'],
    debounceMs: 2000,
    cacheTtlMs: 5000,
    buildSystemPrompt: () => makeAgentSystemPrompt('Meta Orchestrator Agent (conflict resolution only, still advisory)', '{"resolvedConfidence":0-1,"conflicts":["string"],"conflictResolution":"string","confidence":0-1,"missing_data":boolean}'),
    buildUserPrompt: (context) => `Resolve agent conflicts for ${context.contractId}. Trigger=${context.triggerEvent}. Input=${makeCompactContext(context.payload)}`,
    parseOutput: (raw) => {
