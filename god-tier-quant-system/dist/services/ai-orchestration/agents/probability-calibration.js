...
export const probabilityCalibrationAgent = {
    kind: 'probability-calibration',
    preferredModels: AGENT_MODEL_POLICY['probability-calibration'],
    debounceMs: 1200,
    cacheTtlMs: 5000,
    buildSystemPrompt: () => makeAgentSystemPrompt('Probability Calibration Agent (auditor only, no execution authority)', '{"calibrationScore":0-1,"brierScore":0-1,"overconfidenceDetected":boolean,"recommendedAdjustment":-0.2..0.2,"confidence":0-1,"missing_data":boolean}'),
    buildUserPrompt: (context) => `Audit model calibration for ${context.contractId}. Trigger=${context.triggerEvent}. Input=${makeCompactContext(context.payload)}`,
    parseOutput: (raw) => {
