...
export class DigitalImmuneSystemService {
    ...
    start() {
        ...
        this.bus.on(EVENTS.ANOMALY, (event) => {
            ...
            if (event.severity === 'critical' || event.severity === 'high') {
                this.triggerAlert(event.severity === 'critical' ? 'critical' : 'elevated', event.contractId, `anomaly-${event.type}`, event.timestamp);
            }
        });
    ...
    triggerAlert(level, contractId, reason, observedTs) {
