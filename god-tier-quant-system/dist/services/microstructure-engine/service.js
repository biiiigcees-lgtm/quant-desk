...
export class MicrostructureEngine {
    ...
    compute(event) {
        ...
        const spoofProbability = Math.max(0, Math.min(1, sweepProbability * 0.55 +
            spreadExpansionScore * 0.25 +
            (liquidityRegime === 'vacuum' ? 0.2 : 0)));
        const absorptionScore = Math.max(0, Math.min(1, (1 - spreadExpansionScore) * 0.45 +
        ...
        const toxicityScore = Math.max(0, Math.min(1, spoofProbability * 0.5 +
