export class RegimeAdjuster {
    inferRegime(feature, micro) {
        if (micro.panicRepricing)
            return 'panic';
        if (micro.liquidityRegime === 'vacuum')
            return 'low-liquidity';
        if (feature.volatility < 0.01 && micro.spreadExpansionScore < 0.2)
            return 'compression';
        if (feature.volatility > 0.04)
            return 'expansion';
        if (Math.abs(feature.probabilityVelocity) > 0.03 && Math.abs(micro.obiVelocity) > 0.2) {
            return 'momentum-ignition';
        }
        if (Math.abs(feature.probabilityVelocity) < 0.01)
            return 'choppy';
        return feature.probabilityVelocity > 0 ? 'trending' : 'reversal-prone';
    }
    adjustProbability(base, regime) {
        const multiplier = {
            trending: 1.05,
            choppy: 0.95,
            panic: 0.85,
