...
export class RealityLayerService {
    ...
    determineSystemState(hardStop, truthScore, anomalyFactor) {
        if (hardStop || truthScore < 0.20)
            return 'halted';
        if (truthScore < 0.45)
            return 'degraded';
        if (truthScore < 0.70)
    ...
    calculateExecutionPermission(systemState, anomalyFactor) {
        if (systemState === 'halted')
            return false;
        if (systemState === 'degraded' && anomalyFactor <= ANOMALY_FACTOR.high)
            return false;
        return true;
    }
    determineUncertaintyState(truthScore) {
        if (truthScore >= 0.70)
            return 'low';
        if (truthScore >= 0.45)
            return 'medium';
