...
function testBeliefGraphUpdatesFromSnapshot() {
    const bus = new EventBus();
    new BeliefGraphService(bus).start();
    let beliefEvent = null;
    bus.on(EVENTS.BELIEF_GRAPH_STATE, (event) => {
        beliefEvent = event;
    });
    const now = Date.now();
...
function testBeliefGraphDetectsContradictions() {
    const bus = new EventBus();
    new BeliefGraphService(bus).start();
    let beliefEvent = null;
    bus.on(EVENTS.BELIEF_GRAPH_STATE, (event) => {
...
function testBeliefGraphFeedsConstitutionalDecision() {
    const bus = new EventBus();
    new BeliefGraphService(bus).start();
    const constitution = new ConstitutionalDecisionService(bus);
    constitution.start();
    let beliefEvent = null;
    let constitutionalEvent = null;
    bus.on(EVENTS.BELIEF_GRAPH_STATE, (event) => {
        beliefEvent = event;
    });
    ...
    assert.ok(traces.some((t) => t.rule === 'belief-graph-integration'), 'constitutional decision should log belief-graph-integration rule');
}
// Test: Belief graph regime transition hazard
function testBeliefGraphRegimeTransitionHazard() {
    const bus = new EventBus();
    new BeliefGraphService(bus).start();
    let beliefEvent = null;
    bus.on(EVENTS.BELIEF_GRAPH_STATE, (event) => {
...
function testBeliefGraphGraphHealth() {
    const bus = new EventBus();
    new BeliefGraphService(bus).start();
    let beliefEvent = null;
    bus.on(EVENTS.BELIEF_GRAPH_STATE, (event) => {
...
function testConstitutionalDecisionForcesPassiveOnContradictions() {
    const bus = new EventBus();
    ...
    const traces = constitutionalEvent.governance_log;
    assert.ok(traces.some((t) => t.rule.includes('belief') || t.detail.includes('contradiction')), 'constitutional decision should log belief-related adjustments');
}
