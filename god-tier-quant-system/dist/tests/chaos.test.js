...
// ---------------------------------------------------------------------------
function testDuplicateIdempotencyRejection() {
...
// ---------------------------------------------------------------------------
function testContractBusyRejection() {
    const coord = new ExecutionCoordinator({ leaseTtlMs: 5000, idempotencyTtlMs: 30000 });
    const now = 2000000;
...
// ---------------------------------------------------------------------------
function testLeaseTTLExpiry() {
    const leaseTtlMs = 1000;
    const coord = new ExecutionCoordinator({ leaseTtlMs, idempotencyTtlMs: 60000 });
    const t0 = 3000000;
...
// ---------------------------------------------------------------------------
function testIdempotencyTTLExpiry() {
    const idempotencyTtlMs = 5000;
    const coord = new ExecutionCoordinator({ leaseTtlMs: 1000, idempotencyTtlMs });
    const t0 = 4000000;
...
// ---------------------------------------------------------------------------
function testEventBusHistoryBound() {
    const bus = new EventBus(200, 5000);
    for (let i = 0; i < 6000; i++) {
        bus.emit('chaos-event', { i });
    }
    const h = bus.history();
    assert.ok(h.length <= 5000, `EventBus history should be bounded at 5000, got ${h.length}`);
    assert.equal(h.length, 5000, 'EventBus history should be exactly 5000 after overflow');
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function testSafeHandlerCatchesError() {
    let threw = false;
    const throwing = (_event) => {
        threw = true;
        throw new Error('chaos-induced-failure');
    };
    const wrapped = safeHandler(throwing, 'chaos-test');
    assert.doesNotThrow(() => wrapped({ type: 'chaos' }), 'safeHandler must not propagate exceptions from the inner function');
    assert.equal(threw, true, 'inner function must have been called');
}
// ---------------------------------------------------------------------------
// 10. testSafeHandlerTelemetryEmission
// ---------------------------------------------------------------------------
function testSafeHandlerTelemetryEmission() {
    ...
    function safeHandlerWithTelemetry(fn, context, telemetry) {
        return (event) => {
            try {
                fn(event);
            }
    ...
    const wrapped = safeHandlerWithTelemetry((_event) => {
        throw new Error('telemetry-trigger');
    }, 'chaos-telemetry-test', stubBus);
    wrapped({ type: 'chaos' });
...
// ---------------------------------------------------------------------------
function testConcurrentAcquireSameContract() {
    ...
    const coord = new ExecutionCoordinator({ leaseTtlMs: 10000, idempotencyTtlMs: 60000 });
    const now = 5000000;
    const results = Array.from({ length: 5 }, (_, i) => coord.acquire('CONTRACT-CONCURRENT', `key-conc-${i}`, now + i));
...
// ---------------------------------------------------------------------------
function testReleaseThenReacquire() {
    const coord = new ExecutionCoordinator({ leaseTtlMs: 5000, idempotencyTtlMs: 30000 });
    const t0 = 6000000;
    // First acquire
...
// ---------------------------------------------------------------------------
function testSnapshotReducerImmutability() {
    if (!SnapshotReducer) {
...
// ---------------------------------------------------------------------------
function testSnapshotReducerBoundedArrays() {
    if (!SnapshotReducer) {
...
// ---------------------------------------------------------------------------
function testEventBusEmitOrderPreserved() {
    ...
    for (let i = 0; i < h.length; i++) {
        assert.equal(h[i].sequence, i + 1, `event at index ${i} should have sequence ${i + 1}, got ${h[i].sequence}`);
        if (i > 0) {
            assert.ok(h[i].sequence > h[i - 1].sequence, `sequence must be strictly increasing at index ${i}`);
        }
    }
}
// ---------------------------------------------------------------------------
