...
function testConsciousnessContradictionHighEdgeLowTruth() {
    ...
    const hasConflict = last.contradictions.some((c) => c.source === 'probability' && c.target === 'reality');
    assert.ok(hasConflict, 'should detect high-edge vs low-truth contradiction');
}
function testConsciousnessContradictionBeliefSignalMismatch() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
    ...
    const last = events.at(-1);
    const hasMismatch = last.contradictions.some((c) => c.source === 'belief-graph' && c.target === 'signal');
    assert.ok(hasMismatch, 'should detect belief-signal direction mismatch');
}
function testConsciousnessStressCriticalWhenBothHighDensityAndUncertainty() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
    ...
    const last = events.at(-1);
    assert.equal(last.cognitiveStressState, 'critical', `expected critical but got ${last.cognitiveStressState}; density=${last.contradictionDensity}`);
}
function testConsciousnessEmitsOnEverySubscription() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    let count = 0;
    bus.on(EVENTS.SYSTEM_CONSCIOUSNESS, () => { count++; });
