...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function makeMicro(contractId, obi, sweepProbability = 0.3, ts = Date.now()) {
    return {
...
function makeProb(contractId, estimatedProbability, edge, regime = 'trending', ts = Date.now()) {
    return {
        ...
        confidenceInterval: [estimatedProbability - 0.05, estimatedProbability + 0.05],
...
function makeAnomaly(contractId, severity, ts = Date.now()) {
    return { contractId, type: 'test-anomaly', severity, confidenceDegradation: 0.2, details: 'test', timestamp: ts };
}
function makeRealitySnapshot(contractId, truthScore, systemState = 'nominal', ts = Date.now()) {
    return {
        contractId, systemState, actionableState: true, uncertaintyState: 'low',
...
function makeBeliefGraphState(contractId, opts = {}) {
    const contradictions = opts.contradictions ?? [];
    const ts = Date.now();
    const summary = {
        contractId, snapshot_id: 'snap-sc', market_state_hash: 'abc', cycle_id: 'cy-sc',
        beliefAdjustedProbability: 0.55, beliefUncertaintyInterval: [0.5, 0.6],
        contradictions,
...
function makeConstitutionalDecision(contractId, tradeAllowed = true, confidenceScore = 0.75) {
    return {
        ...
        execution_mode: 'market', regime_state: 'trending',
...
function testConsciousnessDoesNotEmitWithoutBothInputs() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.SYSTEM_CONSCIOUSNESS, (e) => { events.push(e); });
...
function testConsciousnessEmitsWhenBothPresent() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
...
function testConsciousnessCriticalWhenHighStress() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
    ...
    assert.equal(last.cognitiveStressState, 'critical', `expected critical but got ${last.cognitiveStressState}`);
}
function testConsciousnessStableWhenHealthy() {
    const bus = new EventBus();
    const svc = new SystemConsciousnessService(bus);
    svc.start();
    const events = [];
    ...
    const last = events.at(-1);
    assert.equal(last.cognitiveStressState, 'stable', `expected stable but got ${last.cognitiveStressState}`);
}
// ─── EpistemicHealthService ───────────────────────────────────────────────────
function testEpistemicGradeAWhenAllHealthy() {
    const bus = new EventBus();
    const svc = new EpistemicHealthService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.EPISTEMIC_HEALTH, (e) => { events.push(e); });
    ...
    const last = events.at(-1);
    assert.ok(last.epistemicHealthScore >= 0.85, `score should be >= 0.85 for grade A; got ${last.epistemicHealthScore}`);
    assert.equal(last.healthGrade, 'A', `grade should be A; got ${last.healthGrade}`);
}
function testEpistemicGradeFWhenCriticalConditions() {
    const bus = new EventBus();
    const svc = new EpistemicHealthService(bus);
    svc.start();
    const events = [];
    ...
    const last = events.at(-1);
    assert.ok(last.epistemicHealthScore < 0.30, `score should be < 0.30 for grade F; got ${last.epistemicHealthScore}`);
    assert.equal(last.healthGrade, 'F', `grade should be F; got ${last.healthGrade}`);
}
function testEpistemicHealthEmitsExecutionControlOnScoreBelow040() {
    const bus = new EventBus();
    const svc = new EpistemicHealthService(bus);
    svc.start();
    const controls = [];
    bus.on(EVENTS.EXECUTION_CONTROL, (e) => { controls.push(e); });
...
function testEpistemicHealthNoRepeatSafeModeEmission() {
    const bus = new EventBus();
    const svc = new EpistemicHealthService(bus);
    svc.start();
    const controls = [];
...
function testAdversarialScorePositiveWithPoorCalibration() {
    const bus = new EventBus();
    const svc = new AdversarialAuditorService(bus);
    svc.start();
    const audits = [];
    bus.on(EVENTS.ADVERSARIAL_AUDIT, (e) => { audits.push(e); });
...
function testAdversarialHiddenRegimeRiskForChoppy() {
    const bus = new EventBus();
    const svc = new AdversarialAuditorService(bus);
    svc.start();
    const audits = [];
...
function testAdversarialHiddenRegimeRiskForLowLiquidity() {
    const bus = new EventBus();
    const svc = new AdversarialAuditorService(bus);
    svc.start();
    const audits = [];
...
function testAdversarialScoreZeroForNominalCleanState() {
    const bus = new EventBus();
    const svc = new AdversarialAuditorService(bus);
    svc.start();
    const audits = [];
...
// ─── MarketMemoryService ──────────────────────────────────────────────────────
function testMarketMemoryDepthIncrementsOnMicrostructure() {
    const bus = new EventBus();
    const svc = new MarketMemoryService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.MARKET_MEMORY, (e) => { events.push(e); });
    for (let i = 0; i < 5; i++) {
...
function testMarketMemoryRecurrenceScorePositiveAfterSimilarFingerprints() {
    const bus = new EventBus();
    const svc = new MarketMemoryService(bus);
    svc.start();
    const events = [];
...
function testMarketMemoryRegimeSignatureContainsFields() {
    const bus = new EventBus();
    const svc = new MarketMemoryService(bus);
    svc.start();
    const events = [];
...
function testMultiTimescaleCoherenceOneWhenAllAgree() {
    const bus = new EventBus();
    const svc = new MultiTimescaleCognitionService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.MULTI_TIMESCALE_VIEW, (e) => { events.push(e); });
...
function testMultiTimescaleCoherenceLowWhenOnlyOneAgrees() {
    const bus = new EventBus();
    const svc = new MultiTimescaleCognitionService(bus);
    svc.start();
    const events = [];
...
function testMultiTimescaleTemporalAlignmentMapping() {
    const bus = new EventBus();
    const svc = new MultiTimescaleCognitionService(bus);
    svc.start();
    const events = [];
    ...
    assert.ok(validAlignments.includes(last.temporalAlignment), `temporalAlignment should be one of ${validAlignments.join(',')}; got ${last.temporalAlignment}`);
}
// ─── Run all ──────────────────────────────────────────────────────────────────
async function run() {
