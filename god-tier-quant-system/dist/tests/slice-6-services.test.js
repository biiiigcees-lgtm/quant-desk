...
// ─── BeliefGraphService ───────────────────────────────────────────────────────
function testBeliefGraphEmitsOnMicrostructure() {
    const bus = new EventBus();
    const service = new BeliefGraphService(bus);
    service.start();
    const events = [];
    bus.on(EVENTS.BELIEF_GRAPH_UPDATE, (e) => { events.push(e); });
...
function testBeliefGraphCalibrationNode() {
    const bus = new EventBus();
    const service = new BeliefGraphService(bus);
    service.start();
    const events = [];
...
function testBeliefGraphAllFourNodeTypes() {
    const bus = new EventBus();
    const service = new BeliefGraphService(bus);
    service.start();
    const events = [];
...
function testBeliefGraphConstitutionalAndConfidenceBounds() {
    const bus = new EventBus();
    const service = new BeliefGraphService(bus);
    service.start();
    const events = [];
    ...
    for (const event of events) {
        ...
        assert.ok(event.constitutionalAdjustment >= -0.09 && event.constitutionalAdjustment <= 0.09, `constitutionalAdjustment ${event.constitutionalAdjustment} should be within [-0.09, 0.09]`);
        assert.ok(event.graphConfidence >= 0 && event.graphConfidence <= 1, `graphConfidence ${event.graphConfidence} should be in [0, 1]`);
    }
}
// ─── StrategyGenomeService ────────────────────────────────────────────────────
function testGenomeNoLifecycleEventWhileInBirth() {
    const bus = new EventBus();
    ...
    genome.start();
    const lifecycle = [];
    bus.on(EVENTS.STRATEGY_LIFECYCLE, (e) => { lifecycle.push(e); });
...
function testGenomeGraduatesToGrowthAfterFivePasses() {
    const bus = new EventBus();
    const ecology = new StrategyEcology(bus);
    ecology.start();
    const genome = new StrategyGenomeService(bus, ecology);
...
function testGenomeDecayOnProlongedPoorPerformance() {
    const bus = new EventBus();
    const ecology = new StrategyEcology(bus);
    ecology.start();
    const genome = new StrategyGenomeService(bus, ecology);
...
function testGenomeExtinctionIsTerminal() {
    const bus = new EventBus();
    const ecology = new StrategyEcology(bus);
    ecology.start();
    const genome = new StrategyGenomeService(bus, ecology);
    ...
    assert.equal(lifecycle.length, countAtExtinction, 'extinct strategy should not emit any further lifecycle events');
}
// ─── SimulationUniverseService ────────────────────────────────────────────────
function testSimUniverseEmitsBothValidationsOnSignal() {
    const bus = new EventBus();
    const sim = new SimulationUniverseService(bus);
    sim.start();
    const validations = [];
    bus.on(EVENTS.VALIDATION_RESULT, (e) => { validations.push(e); });
...
function testSimUniverseWalkForwardPassOnConfidentSignal() {
    const bus = new EventBus();
    const sim = new SimulationUniverseService(bus);
    sim.start();
    const validations = [];
...
function testSimUniverseAdversarialPanicPenalty() {
    const bus = new EventBus();
    const sim = new SimulationUniverseService(bus);
    sim.start();
    const validations = [];
...
function testSimUniverseEmitsUniverseOnAggregatedSignal() {
    const bus = new EventBus();
    const sim = new SimulationUniverseService(bus);
    sim.start();
    const universe = [];
    bus.on(EVENTS.SIMULATION_UNIVERSE, (e) => { universe.push(e); });
    ...
    assert.ok(universe[0].mirrorConfidence >= 0 && universe[0].mirrorConfidence <= 1, `mirrorConfidence ${universe[0].mirrorConfidence} should be in [0, 1]`);
}
function testSimUniverseMirrorEmitsAllFourCandidates() {
    const bus = new EventBus();
    const sim = new SimulationUniverseService(bus);
    sim.start();
    const mirrors = [];
    bus.on(EVENTS.EXECUTION_PATH_MIRROR, (e) => { mirrors.push(e); });
