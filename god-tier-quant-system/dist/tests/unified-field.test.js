...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function makeMarketData(contractId, yesPrice = 0.55, overrides) {
    return {
        contractId,
        yesPrice,
        noPrice: 1 - yesPrice,
...
function makeMicro(contractId, obi = 0.3, overrides) {
    return {
        contractId,
        obi,
        obiVelocity: 0.1,
...
function makeFeature(contractId, pVelocity = 0.02, overrides) {
    return {
        contractId,
...
function makeProb(contractId, implied = 0.6) {
    return {
        contractId,
...
function testOrderbookDeltaEmitsOnFirstEvent() {
    const bus = new EventBus();
    const svc = new OrderbookDeltaService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.ORDERBOOK_DELTA, (e) => { events.push(e); });
...
function testOrderbookDeltaDetectsAddedBid() {
    const bus = new EventBus();
    const svc = new OrderbookDeltaService(bus);
    svc.start();
    const events = [];
...
function testOrderbookDeltaDetectsRemovedBid() {
    const bus = new EventBus();
    const svc = new OrderbookDeltaService(bus);
    svc.start();
    const events = [];
...
function testOrderbookDeltaDetectsSpoofedBid() {
    const bus = new EventBus();
    const svc = new OrderbookDeltaService(bus);
    svc.start();
    const events = [];
...
function testLiquidityGravityAskWallBias() {
    const bus = new EventBus();
    const svc = new LiquidityGravityService(bus);
    svc.start();
    const events = [];
...
function testRegimeTransitionEmits() {
    const bus = new EventBus();
    const svc = new RegimeTransitionService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.REGIME_TRANSITION, (e) => { events.push(e); });
...
function testRegimeTransitionPanicRegime() {
    const bus = new EventBus();
    const svc = new RegimeTransitionService(bus);
    svc.start();
    const events = [];
...
function testRegimeTransitionInstabilityRange() {
    const bus = new EventBus();
    const svc = new RegimeTransitionService(bus);
    svc.start();
    const events = [];
    ...
    for (const e of events) {
        assert.ok(e.regimeInstability >= 0 && e.regimeInstability <= 1, `regimeInstability ${e.regimeInstability} must be in [0,1]`);
    }
}
// ─── NoiseFilterService ───────────────────────────────────────────────────────
function testNoiseFilterEmits() {
    const bus = new EventBus();
    const svc = new NoiseFilterService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.FILTERED_SIGNAL, (e) => { events.push(e); });
...
function testNoiseFilterStructuralFractionRange() {
    const bus = new EventBus();
    const svc = new NoiseFilterService(bus);
    svc.start();
    const events = [];
    ...
    for (const e of events) {
        assert.ok(e.structuralFraction >= 0 && e.structuralFraction <= 1, `structuralFraction ${e.structuralFraction} must be [0,1]`);
        assert.ok(e.manipulationScore >= 0 && e.manipulationScore <= 1, `manipulationScore must be [0,1]`);
    }
}
// ─── UnifiedMarketFieldService ────────────────────────────────────────────────
function testUnifiedFieldEmitsOnMicroAndFeatures() {
    const bus = new EventBus();
    const svc = new UnifiedMarketFieldService(bus);
    svc.start();
    const events = [];
    bus.on(EVENTS.UNIFIED_FIELD, (e) => { events.push(e); });
...
function testUnifiedFieldBullishWhenStrongBullishForces() {
    const bus = new EventBus();
    const svc = new UnifiedMarketFieldService(bus);
    svc.start();
    const events = [];
...
function testUnifiedFieldNoBetWhenHighRegimeInstability() {
    const bus = new EventBus();
    const svc = new UnifiedMarketFieldService(bus);
    svc.start();
    const events = [];
...
function testUnifiedFieldCausalAttributionSumsToApprox1() {
    const bus = new EventBus();
    const svc = new UnifiedMarketFieldService(bus);
    svc.start();
    const events = [];
    ...
    assert.ok(Math.abs(sum - 1) < 0.01 || sum <= 1, `directional causal attribution should sum to ~1; got ${sum}`);
}
function testUnifiedFieldGetLatestField() {
    const bus = new EventBus();
    const svc = new UnifiedMarketFieldService(bus);
    svc.start();
...
function testRealityAlignmentInitialWeightsSumTo1() {
    const bus = new EventBus();
...
function testRealityAlignmentUpdatesWeightsAfterProbability() {
    const bus = new EventBus();
...
// ─── CausalWeightEngine ───────────────────────────────────────────────────────
function testCausalWeightEngineRespondsToAlignment() {
    const bus = new EventBus();
    const uField = new UnifiedMarketFieldService(bus);
    const ra = new RealityAlignmentService(bus);
...
function testShadowTradingEmitsThreeStrategies() {
    const bus = new EventBus();
    const uField = new UnifiedMarketFieldService(bus);
    ...
    shadow.start();
    const events = [];
    bus.on(EVENTS.SHADOW_DECISION, (e) => { events.push(e); });
...
function testShadowTradingConservativeMoreRestrictive() {
    const bus = new EventBus();
    const uField = new UnifiedMarketFieldService(bus);
    const shadow = new ShadowTradingService(bus);
    uField.start();
    ...
    assert.ok(conservative.threshold > aggressive.threshold, `conservative threshold (${conservative.threshold}) should be > aggressive threshold (${aggressive.threshold})`);
}
function testShadowTradingHasValidBestStrategy() {
    const bus = new EventBus();
    const uField = new UnifiedMarketFieldService(bus);
    const shadow = new ShadowTradingService(bus);
    uField.start();
...
function testFullPipelineProducesUnifiedFieldDecision() {
    const bus = new EventBus();
    ...
    const fieldEvents = [];
    const shadowEvents = [];
    bus.on(EVENTS.UNIFIED_FIELD, (e) => { fieldEvents.push(e); });
    bus.on(EVENTS.SHADOW_DECISION, (e) => { shadowEvents.push(e); });
    ...
    assert.ok(['ABOVE', 'BELOW', 'NO BET'].includes(lastField.decision), `decision must be ABOVE/BELOW/NO BET; got '${lastField.decision}'`);
    assert.ok(lastField.pAbove + lastField.pBelow + lastField.pNoBet > 0.99, 'probabilities must sum to ~1');
}
// ─── Run all ──────────────────────────────────────────────────────────────────
async function run() {
