...
import type { MarketDataEvent, LiquidityGravityEvent } from '../../core/schemas/events.js';
...
export class LiquidityGravityService {
  private readonly state = new Map<string, GravityState>();
  ...
  private process(e: MarketDataEvent): void {
    ...
    const resistanceZones = [
      ...bidWalls.map(w => ({ price: w.price, strength: clamp(w.size / 1000, 0, 1), side: 'bid' as const })),
      ...askWalls.map(w => ({ price: w.price, strength: clamp(w.size / 1000, 0, 1), side: 'ask' as const })),
    ];
    ...
    const event: LiquidityGravityEvent = {
    ...
    this.bus.emit<LiquidityGravityEvent>(EVENTS.LIQUIDITY_GRAVITY, event);
  }
}
