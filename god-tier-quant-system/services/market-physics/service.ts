...
export class MarketPhysicsService {
  private readonly state = new Map<string, PhysicsState>();
  private readonly latest = new Map<string, MarketPhysicsEvent>();
  ...
  getLatest(contractId: string): MarketPhysicsEvent | undefined {
    return this.latest.get(contractId);
  }

  private getState(contractId: string): PhysicsState {
  ...
  private emit(contractId: string, timestamp: number): void {
    ...
    const compression = clamp(
      ...
      (1 - state.feature.volatility) * 0.2,
      0,
      1,
    );

    const expansion = clamp(
      state.micro.spreadExpansionScore * 0.45 +
      Math.min(1, Math.abs(state.micro.obiVelocity)) * 0.35 +
      state.feature.volatility * 0.2,
      0,
      1,
    );

    const inertia = clamp(
      Math.min(1, Math.abs(state.feature.probabilityVelocity)) * 0.5 +
      Math.min(1, Math.abs(state.micro.obi)) * 0.35 +
      Math.min(1, Math.abs(state.feature.pressureAcceleration)) * 0.15,
      0,
      1,
    );

    const exhaustion = clamp(
      state.feature.volatility * 0.45 +
    ...
    const entropyExpansion = clamp(
      (state.drift?.kl ?? 0) * 0.55 +
    ...
    const structuralStress = clamp(
      expansion * 0.35 + exhaustion * 0.3 + entropyExpansion * 0.25 + (1 - liquidityConservation) * 0.1,
      0,
      1,
    );

    const event: MarketPhysicsEvent = {
      contractId,
    ...
    this.bus.emit<MarketPhysicsEvent>(EVENTS.MARKET_PHYSICS, event);
