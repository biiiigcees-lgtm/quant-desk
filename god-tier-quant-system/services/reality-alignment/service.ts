...
function normalizeWeights(w: { liquidity: number; flow: number; volatility: number; entropy: number }) {
  const total = w.liquidity + w.flow + w.volatility + w.entropy;
  if (total < 0.001) return { ...INITIAL_WEIGHTS };
  return {
...
export class RealityAlignmentService {
  private readonly contractState = new Map<string, AlignmentState>();
  ...
  private align(e: ProbabilityEvent): void {
    const state = this.getOrInit(e.contractId);
    ...
    if (currentPriceSign === 0 || state.pending.length === 0) return;

    // Match against most recent prediction
    const pred = state.pending[state.pending.length - 1]!;
    const actualOutcome: 'ABOVE' | 'BELOW' | 'UNKNOWN' =
      currentPriceSign > 0 ? 'ABOVE' : currentPriceSign < 0 ? 'BELOW' : 'UNKNOWN';
    ...
    const event: RealityAlignmentEvent = {
    ...
    this.bus.emit<RealityAlignmentEvent>(EVENTS.REALITY_ALIGNMENT, event);
    void prevSign;
  }

  getWeights(contractId: string) {
    return this.contractState.get(contractId)?.weights ?? { ...INITIAL_WEIGHTS };
  }

  private getOrInit(contractId: string): AlignmentState {
