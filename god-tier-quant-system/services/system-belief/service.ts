...
export class SystemBeliefService {
  ...
  private calculateRegimeData(snapshot: DecisionSnapshotEvent, inputs: ContractInputs): any {
  ...
  private calculateStructuralData(snapshot: DecisionSnapshotEvent, inputs: ContractInputs): any {
  ...
  private calculateBiasData(p: number, current: SystemBeliefV2, regimeStability: number): any {
    ...
    const nextPersistence = clamp(
      current.directionalBiasModel.persistence * 0.8 + regimeStability * 0.2,
      0, 1,
    );
  ...
  private calculateBehavioralData(snapshot: DecisionSnapshotEvent, inputs: ContractInputs, edge: number): any {
    const micro = inputs.microstructure;
    const anomaly = inputs.anomaly;
    const uncertainty = snapshot.state.probability.uncertaintyScore;

    const expectedVolatility = clamp(
      (uncertainty * 0.55) + (micro?.spreadExpansionScore ?? 0) * 0.25 + (anomaly ? 0.2 : 0),
      0, 1,
    );
  ...
  private calculateAssessmentData(snapshot: DecisionSnapshotEvent, inputs: ContractInputs, edge: number): any {
    const drift = inputs.drift;
    const uncertainty = snapshot.state.probability.uncertaintyScore;
    ...
    const reliabilityTarget = clamp(
      1 - uncertainty * 0.4 - (drift?.severity === 'high' ? 0.2 : 0),
      0, 1,
    );
    const calibrationTarget = clamp(
      calibError + (drift?.psi ?? 0) * 0.15,
      0, 1,
    );
  ...
  private buildBeliefState(current: SystemBeliefV2, regimeData: any, structuralData: any, biasData: any, behavioralData: any, assessmentData: any): SystemBeliefV2 {
    return {
      regimeHypothesis: {
