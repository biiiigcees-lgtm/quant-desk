...
async function testJournalSuccessfulExecution(): Promise<void> {
  const coord = new ExecutionCoordinator({ leaseTtlMs: 5_000, idempotencyTtlMs: 30_000 });
...
async function testJournalExecutionErrorRollback(): Promise<void> {
  const coord = new ExecutionCoordinator({ leaseTtlMs: 5_000, idempotencyTtlMs: 30_000 });
  const journal = new ExecutionJournal(coord);
...
async function testJournalDuplicateIdempotencyKey(): Promise<void> {
  const coord = new ExecutionCoordinator({ leaseTtlMs: 5_000, idempotencyTtlMs: 30_000 });
  const journal = new ExecutionJournal(coord);
...
function testInvariantCheckerValidSnapshot(): void {
...
function testInvariantCheckerProbabilityOutOfRange(): void {
  const checker = new InvariantChecker();
  const reducer = new SnapshotReducer2();
...
function testInvariantCheckerCIOrdering(): void {
  const checker = new InvariantChecker();
  const reducer = new SnapshotReducer2();
...
function testInvariantCheckerCustomRule(): void {
  const checker = new InvariantChecker();
  ...
  for (let i = 0; i < 5; i++) snap = reducer.apply('probability', {});
  const violations = checker.check(snap);
  assert.ok(violations.some((v) => v.invariant === 'MAX_SEQUENCE_5'), 'custom invariant should fire');
}

function testInvariantCheckerViolationSummary(): void {
  const checker = new InvariantChecker();
  const reducer = new SnapshotReducer2();
