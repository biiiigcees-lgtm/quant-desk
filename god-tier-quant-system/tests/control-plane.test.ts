...
function testOrderedHistory(): void {
  ...
  assert.deepEqual(
    ...
    [EVENTS.MARKET_DATA, EVENTS.PROBABILITY],
    'tracked events should preserve emit order',
  );
  assert.deepEqual(
    records.map((record) => record.sequence),
    [1, 2],
    'bus sequence numbers should be monotonic',
  );
  assert.equal(replay.checksum(), replay.checksum(), 'checksum should remain stable for unchanged history');
}

function testHardStopRiskBlocksExecution(): void {
  const bus = new EventBus();
  new AdaptiveRiskEngine(bus, 10000, 0.5).start();
  new ExecutionIntelligenceEngine(bus).start();

  const controlModes: string[] = [];
  const executionStates: Array<{ phase: string; reason: string }> = [];
  let decisionApproved = true;
  let decisionReason = '';

  bus.on(EVENTS.EXECUTION_CONTROL, (event: { mode: string }) => {
    controlModes.push(event.mode);
  });
  bus.on(EVENTS.RISK_DECISION, (event: { approved: boolean; reason: string }) => {
...
async function testValidationBlockingRemovesWeights(): Promise<void> {
  const bus = new EventBus();
  ...
  const aggregated: Array<{ direction: string; score: number }> = [];
  bus.on(EVENTS.AGGREGATED_SIGNAL, (event: { direction: string; score: number }) => {
