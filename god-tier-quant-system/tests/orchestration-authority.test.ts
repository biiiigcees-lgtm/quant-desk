...
function testAiCannotTriggerExecutionDirectly(): void {
  ...
  assert.equal(
    executionPlanCount,
    0,
    'AI aggregated intelligence must not produce execution plans without deterministic risk decision',
  );
}

function testHardStopStillBlocksEvenWithAiAdvice(): void {
  const bus = new EventBus();
  new ExecutionIntelligenceEngine(bus).start();
  ...
  bus.on(EVENTS.EXECUTION_STATE, (event: { phase: string }) => {
...
function testAiRiskAdviceIsBoundedToSizing(): void {
  const bus = new EventBus();
  new AdaptiveRiskEngine(bus, 10_000, 0.02).start();

  const riskDecisions: Array<{ size: number; approved: boolean }> = [];
  bus.on(EVENTS.RISK_DECISION, (event: { size: number; approved: boolean }) => {
