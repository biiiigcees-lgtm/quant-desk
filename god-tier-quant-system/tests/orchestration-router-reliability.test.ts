...
function makeSnapshot(snapshotId: string, now: number): DecisionSnapshotEvent {
  const marketData = {
    contractId: 'KXBTC-REL',
    yesPrice: 0.5,
    noPrice: 0.5,
    spread: 0.01,
    bidLevels: [[0.49, 100]] as Array<[number, number]>,
    askLevels: [[0.51, 100]] as Array<[number, number]>,
    volume: 1000,
    timestamp: now,
  };
  ...
  const probability = {
    ...
    edge: 0.02,
    confidenceInterval: [0.48, 0.56] as [number, number],
