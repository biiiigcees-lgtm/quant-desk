...
import type { RiskGovernanceEvent } from '../core/schemas/events.js';

// ─── Helpers ──────────────────────────────────────────────────────────────────

function makeEpistemicHealth(contractId: string, score: number) {
  ...
  return {
    ...
    healthGrade: grade as 'A' | 'B' | 'C' | 'D' | 'F',
    timestamp: Date.now(),
  };
}

function makeAnomaly(contractId: string, severity: 'low' | 'medium' | 'high' | 'critical') {
  return {
...
function makeExecutionControl(mode: 'normal' | 'safe-mode' | 'hard-stop', reason = 'test') {
  return { mode, reason, timestamp: Date.now() };
}

function makeProb(contractId: string, ts = Date.now()) {
  return {
    contractId, estimatedProbability: 0.55,
    marketImpliedProbability: 0.5, edge: 0.05,
    confidenceInterval: [0.5, 0.6] as [number, number],
    uncertaintyScore: 0.3, calibrationError: 0.05,
    brierScore: 0.2, regime: 'trending' as const, timestamp: ts,
  };
}

function makeExecutionPlan(contractId: string, executionId = 'exec-1') {
  return {
    executionId, contractId, direction: 'YES' as const,
    orderStyle: 'market' as const, slices: 1,
    expectedSlippage: 0.003, fillProbability: 0.9,
    limitPrice: 0.55, size: 100, latencyBudgetMs: 70,
    routeReason: 'test', safetyMode: 'normal' as const, timestamp: Date.now(),
  };
}

// ─── LogicalClock ─────────────────────────────────────────────────────────────

function testClockGlobalTickMonotonic(): void {
...
function testClockPerContractIndependence(): void {
  const clock = new LogicalClock();
  clock.tick('A');
  clock.tick('A');
  clock.tick('B');
...
function testClockSnapshotIdUnique(): void {
  const clock = new LogicalClock();
...
function testGovernorStartsNormal(): void {
...
function testGovernorDegradedOnWeakEpistemic(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();

  const modes: string[] = [];
  bus.on<RiskGovernanceEvent>(EVENTS.RISK_GOVERNANCE, (e) => { modes.push(e.mode); });
...
function testGovernorSafeOnLowEpistemic(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorLockedOnCriticalEpistemic(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorLockedIsSticky(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorSafeOnExecutionControlSafeMode(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorLockedOnHardStop(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorDegradedOnCriticalAnomaly(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorPriorityHigherModeWins(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testGovernorSubscriberNotified(): void {
  const bus = new EventBus();
  const gov = new RiskGovernor(bus);
  gov.start();
...
function testMemoryLifecyclePruneCallsRegistrations(): void {
...
function testMemoryLifecycleUnregister(): void {
  const mgr = new MemoryLifecycleManager();
  let called = 0;
...
function testMemoryLifecycleErrorInPrunerDoesNotCrash(): void {
  const mgr = new MemoryLifecycleManager();
  mgr.register('throws', () => { throw new Error('prune-error'); });
  mgr.register('good', () => { /* ok */ });
  assert.doesNotThrow(() => mgr.prune(), 'prune errors must be swallowed');
}

function testMemoryLifecycleStartStopIdempotent(): void {
  const mgr = new MemoryLifecycleManager();
  mgr.start(1000);
...
function testLineageTracerOpenChainOnProbability(): void {
  const bus = new EventBus();
...
function testLineageTracerRecordsAgentResponses(): void {
  const bus = new EventBus();
  const tracer = new EventLineageTracer(bus);
  tracer.start();
...
function testLineageTracerRecordsExecutionDecision(): void {
  const bus = new EventBus();
  const tracer = new EventLineageTracer(bus);
  tracer.start();
...
function testLineageTracerBoundedAtMaxChains(): void {
  const bus = new EventBus();
...
function testLineageTracerPruneOlderThan(): void {
  const bus = new EventBus();
  const tracer = new EventLineageTracer(bus);
  tracer.start();
