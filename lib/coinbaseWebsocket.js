...
export class CoinbaseWS {
  ...
  connect() {
    ...
    this.ws.onopen = () => {
      ...
      marketStore.setState({ wsConnected: true, reconnectCount: 0, lastMsgTs: this.lastMsgTs });
  ...
  _startFallbackWS() {
    ...
    this.fallbackWs.onopen = () => {
      ...
      marketStore.setState({ wsConnected: true, lastMsgTs: this.fallbackLastMsgTs });
      this._setStatus('connected');
  ...
  _handleFallbackMessage(msg) {
    ...
    if (channel === 'ticker' && payload) {
      ...
      marketStore.setState({ wsConnected: true, lastMsgTs: now });
      this.fallbackLastMsgTs = now;
    }
  }
