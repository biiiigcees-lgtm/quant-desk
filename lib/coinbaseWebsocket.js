...
export class CoinbaseWS {
  ...
  connect() {
    ...
    this.ws.onclose = () => {
      ...
      clearInterval(this.staleTimer);
      clearInterval(this.restPollTimer);
      if (!this.fallbackActive && !this.manualFallback) this._startFallbackWS();
      this._scheduleReconnect();
    };
  }
  ...
  setManualFallback(enable) {
    this.manualFallback = !!enable;
    marketStore.setState({ manualFallbackEnabled: this.manualFallback });
    if (this.manualFallback) {
      this._startFallbackWS();
      try { this.ws?.close(); } catch(e) {}
    } else {
