...
export class MarketStore {
  constructor() {
    this._state = {
      ...
      // WS meta
      wsConnected: false, wsLatency: 0, reconnectCount: 0,
      updatedAt: null,
  ...
  subscribe(fn) {
    ...
    try { fn(this._state); } catch(e) {}     // immediate call with current state
    return () => this._listeners.delete(fn); // returns unsubscribe
  }

  _notify() {
    for (const fn of this._listeners) {
      try { fn(this._state); } catch(e) { /* isolate listener errors */ }
    }
  }
  ...
  computeLiquidationZones(price) {
    ...
    const zones = [
      ...
      { price: price * 0.975, strength: 0.70, side: 'short', leverage: '5x'  },
      { price: price * 1.050, strength: 0.40, side: 'long',  leverage: '3x'  },
      { price: price * 0.950, strength: 0.45, side: 'short', leverage: '3x'  },
      { price: price * 1.100, strength: 0.25, side: 'long',  leverage: '2x'  },
      { price: price * 0.900, strength: 0.30, side: 'short', leverage: '2x'  },
    ];
