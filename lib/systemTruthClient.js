...
export function getDirectionalDecision(input = {}) {
  ...
  const directionalEdge = Math.abs((Number.isFinite(pDecisionAbove) ? pDecisionAbove : 0.5) - 0.5) * 2;
  ...
  const verdict = (Number.isFinite(pDecisionAbove) ? pDecisionAbove : 0.5) >= 0.5 ? 'ABOVE' : 'BELOW';
...
export function deriveSystemTruth(input = {}) {
  ...
  const direction = !decision.secureDecision
    ? 'NEUTRAL'
    : decision.verdict === 'ABOVE'
      ? 'UP'
      : 'DOWN';
  ...
  return {
    ...
    executionAllowed: decision.secureDecision && direction !== 'NEUTRAL' && riskLevel !== 'HIGH' && riskLevel !== 'CRITICAL',
    riskLevel,
