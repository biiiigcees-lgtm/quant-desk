export function generateNarrative(observation) {
    const confidenceBand = Math.abs(observation.signalScore) > 70
        ? 'high'
        : Math.abs(observation.signalScore) > 40
            ? 'medium'
            : 'low';
    const summary = `Signal ${observation.signalDirection} at score ${observation.signalScore.toFixed(1)} ` +
